...
def main():
    ...
    for gp in all_gps:
        for d in all_days:
            ...
            model.add_at_most_one(basic_wards+art_ward)
    ...
    for gp in all_gps:
        ...
        basic_wards_hrs_values = [duty_shifts[(gp,d,w,t)]*hrs_per_week[days_of_week[get_day_of_week(d)]] if not is_w4_weekday_signeds(d,w,t) else duty_shifts[(gp,d,w,duty_types[0])]*hrs_per_week[days_of_week[get_day_of_week(d)]] for d in range(len(prev_duty_shift),num_days) for w in all_wards for t in duty_types]
        ...
        basic_wards_signed_hrs_values = [duty_shifts[(gp,d,w,duty_types[1])]*hrs_per_week[days_of_week[get_day_of_week(d)]] for d in range(len(prev_duty_shift),num_days) for w in all_wards if not is_w4_weekday_signeds(d,w,duty_types[1]) ]
        model.add(cal_gp_signed_hrs[gp]==sum(basic_wards_signed_hrs_values))
    ...
    model.add(compensation_minimizer==sum(comp_gp_value_list))
    lovers_count_minimizer = model.new_int_var(0,num_days,f"lovers_count_minimizer")
    model.add(lovers_count_minimizer==count_gp_15_assigned-count_gp_15_11_together)
    model.minimize(4*fairness_minimizer + compensation_minimizer + 4*lovers_count_minimizer)
    #solver logic
    ...
    if status == cp_model.OPTIMAL or status == cp_model.FEASIBLE:
        ...
        print("Max hr", solver.value(max_hr),"; Min hr", solver.value(min_hr),"; Avg hr", solver.value(avg_hr))
        for gp in all_gps:
...
if __name__ == "__main__":
    all_wards = ['W4M','W4F','W9','MDR','IMW','ART']
    duty_types = ['actual','signed']
    duty_solution = main()
